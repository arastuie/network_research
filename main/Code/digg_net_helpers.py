...
def get_lp_resutls(gather_individual_lp_results=False):
    if gather_individual_lp_results:
        ...
        # loading result data
        for result_file in os.listdir(digg_results_lp_file_path + 'results'):

            with open(digg_results_lp_file_path + 'results/' + result_file, 'rb') as f:
        ...
        with open(digg_results_lp_file_path + "cumulated_results/all-6-methods-results.pckle", 'wb') as f:
            pickle.dump([car, cclp, aa, dcaa, cn, dccn], f, protocol=-1)
    else:
        with open(digg_results_lp_file_path + "cumulated_results/all-6-methods-results.pckle", 'rb') as f:
            car, cclp, aa, dcaa, cn, dccn = pickle.load(f)

    lph.calculate_lp_perfomace(car, cclp, aa, dcaa, cn, dccn)
