...
def degree_dist():
    ...
    in_degree_count = np.unique(in_degree, return_counts=True)
    in_degree_count = h.log_binning(in_degree_count, 100)

    out_degree = np.array(out_degree) + 1
    out_degree_count = np.unique(out_degree, return_counts=True)
    out_degree_count = h.log_binning(out_degree_count, 100)

    plt.xscale('log')
    plt.yscale('log')
    plt.scatter(in_degree_count[0], in_degree_count[1], c='r', marker='.', alpha=0.5, label="Global in-degree")
    plt.scatter(out_degree_count[0], out_degree_count[1], c='black', marker='d', alpha=0.5, label="Global out-degree")
    ...
    # plt.tight_layout()
    # # plt.savefig('{}local-degree-dist/plots-1/local-in-degree-dist.pdf'.format(directed_local_degree_empirical_base_results_path), format='pdf')
    # plt.clf()
    #
    # bins = np.logspace(np.log10(1), np.log10(max(in_degree)), 50)
    # plt.hist(in_degree, bins=bins, density=True, log=True)
    ...
    # plt.ylabel('Density', fontsize=20)
    # plt.tight_layout()
    # plt.savefig('{}local-degree-dist/plots-1/global-in-degree-dist.pdf'.format(directed_local_degree_empirical_base_results_path), format='pdf')
    # plt.clf()
    #
    ...
    # plt.ylabel('Density', fontsize=20)
    # plt.tight_layout()
    # plt.savefig('{}local-degree-dist/plots-1/global-out-degree-dist.pdf'.format(directed_local_degree_empirical_base_results_path), format='pdf')
    # plt.clf()