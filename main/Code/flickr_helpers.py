...
def extract_ego_nets(n_egonets, max_n_nodes_in_egonet=100000):
    ...
    while n_egonets_extracted <= n_egonets and cnt < len(nodes_to_analyze):
        ...
        with open('{0}{1}.pckl'.format(egonet_files_path, nodes_to_analyze[cnt]), 'wb') as f:
...
def create_multiple_egonets(n, batch_size, n_cores=10):
    ...
    for node in ego_nodes:
        if os.path.isfile('{0}{1}.pckl'.format(egonet_files_path, node)):
...
def read_multiple_ego_flickr_graphs(ego_node_list):
    ...
    for ego_node in egonets:
        ...
        with open('{0}{1}.pckl'.format(egonet_files_path, ego_node), 'wb') as f:
