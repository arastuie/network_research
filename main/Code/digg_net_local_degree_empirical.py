    ...
    for ego in nodes_in_snap:
        ...
        for i in range(len(ego_snaps) - 1):
            ...
            for v in formed_v_nodes:
                temp_z_local_formed.append(len(set(nx.neighbors(ego_snaps[i], v)).intersection(z_nodes)))
                temp_z_global_formed.append(nx.degree(ego_snaps[i], v))

            for v in not_formed_v_nodes:
                temp_z_local_not_formed.append(len(set(nx.neighbors(ego_snaps[i], v)).intersection(z_nodes)))
                temp_z_global_not_formed.append(nx.degree(ego_snaps[i], v))

            results[i][ego] = {
                'num_nodes': ego_snaps[i].number_of_nodes(),
                'num_z_nodes': len(z_nodes),
                'local_degrees_formed': temp_z_local_formed,
                'local_degrees_not_formed': temp_z_local_not_formed,
                'global_degrees_formed': temp_z_global_formed,
                'global_degrees_not_formed': temp_z_global_not_formed,
            }

        analyzed_nodes.add(ego)

# save an empty file in analyzed_egonets to know which ones were analyzed
with open('/shared/Results/EgocentricLinkPrediction/main/empirical/digg/pickle-files/{0}-days-duration-results.pckle'
